...
def fetch_recent_data(measure_id, weeks):
    """Fetch the data of last few weeks"""

    assert weeks >= 1  # guarantee a week's spacing

    # Current time (UTC)
    now = datetime.datetime.utcnow()

    # Start time for data
    start = now - datetime.timedelta(days=7 * weeks)
    ...
    for measure in data['items']:
        ...
        if d.replace(tzinfo=None) < now - datetime.timedelta(days=6):
            # avoid corrupted data
            try:
                if not type(measure['value']) is float: # cannot believe there are corrupted data like this... a mixture of list and float....
                    return [], []
            except:
                return [], []
            # Append data
